...
- `_detect_billing_mode` distinguishes API key from subscription Bearer tokens: `Bearer sk-ant-*` → `anthropic_api` (API key sent as Bearer); upstream not `api.anthropic.com` with any other Bearer → `anthropic_api` (enterprise gateway, not a claude.ai subscription endpoint); `Bearer <other>` on `api.anthropic.com` → `anthropic_subscription` (OAuth token). This covers both API keys mis-sent as Bearer and third-party gateways (Capgemini, etc.) whose Bearer tokens are enterprise credentials rather than claude.ai OAuth.
...
- `./.token-usage.ddbya` and `./.pricing.ddbya` are both committed to this repository. `.pricing.ddbya` is the master pricing reference, used as the default when ddbya is deployed elsewhere.
