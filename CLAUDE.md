...
- Model name: defaults to whatever the response events report (so Anthropic's date-resolution like `sonnet-latest` → `sonnet-20241022` is preserved in the log).
- `billing_mode` values: `anthropic_subscription` (Claude.ai OAuth Bearer token, all-you-can-eat), `anthropic_subscription_overage` (subscription tipped into pay-per-use), `anthropic_api` (Anthropic Console API key, always pay-per-use), `deepseek_api` (DeepSeek API via `--deepseek` flag), `aws_bedrock` (AWS Bedrock via LLM gateway), `google_vertex` (Google Vertex AI via LLM gateway), `azure_foundry` (Microsoft Azure Foundry via LLM gateway).
