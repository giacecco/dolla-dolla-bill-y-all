...
- `TokenLogger` — thread-safe JSONL writer with in-memory session tracking. Writes to `.ddbya.d/usage-<identity>.ddbya` (see Identity & layout).
...
- `_migrate_legacy_layout(project_dir, identity)` — idempotent startup migration. Moves `.token-usage.ddbya` → `.ddbya.d/usage-<identity>.ddbya` and `.pricing.ddbya` → `.ddbya.d/pricing.ddbya` if the legacy files exist at the project root. Prints one line to stderr per file moved.
...
<project>/
  .ddbya.d/
    ...
    usage-alice-example.com.ddbya   ← Alice's token log
    usage-bob-example.com.ddbya     ← Bob's token log
```

Per-user filenames use the pattern `usage-<identity>.ddbya`. Identity is resolved at startup via `_resolve_identity()`: `git config user.email` → `$USER` → UUID stored in `~/.config/ddbya/id`. The identity is sanitised to lowercase `[a-z0-9._-]` (any other character collapsed to `-`).
