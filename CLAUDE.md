...
- `_detect_billing_mode` distinguishes API key from subscription Bearer tokens by inspecting the token value: `Bearer sk-ant-*` → `anthropic_api`; any other Bearer token → `anthropic_subscription`. This is necessary because Claude Code sends API keys as `Authorization: Bearer sk-ant-...` rather than `x-api-key`. Some enterprise gateways (e.g. `anthropic.generative.engine.capgemini.com`) also send Bearer tokens that should be treated as API billing.
