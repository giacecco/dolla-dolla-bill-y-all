...
- `maybe_update_pricing(project_dir, force, interactive)` — checks `.pricing.ddbya` staleness on startup. If absent or > 30 days old, prompts the user (interactive sessions only). With `force=True`, fetches unconditionally. On fetch, runs `claude -p --model <haiku> <prompt>` directly (not through the proxy) with web search to retrieve current tariffs from `anthropic.com/pricing`. Merges results into `.pricing.ddbya`: unchanged prices bump the `to` date; changed prices seal the old record and append a new one. The JSONL format is `{provider, model_prefix, from, to, input, output, cache_read, cache_write, fetched_at}`.
...
- `./.token-usage.ddbya` and `./.pricing.ddbya` are project-local runtime data — do not commit them.
