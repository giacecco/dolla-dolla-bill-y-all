...
- `parse_args()` — extracts `-o`/`--ollama-model`, `-l`/`--limit`, `-t`/`--tag`, and `--last` from argv. Returns `(ollama_model, limit_usd, lookback_days, tags, claude_args)`. Validates that `--limit` and `--last` are paired. `-t`/`--tag` can be given multiple times.
