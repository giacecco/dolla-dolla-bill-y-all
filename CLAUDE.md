...
- `parse_args()` — extracts `-o`/`--ollama-model`, `--deepseek`, `-l`/`--limit`, `-t`/`--tag`, `--last`, `--list-tags`, and `--pricing` from argv. Returns `(ollama_model, deepseek, limit_usd, lookback_days, tags, list_tags, force_pricing, claude_args)`. Validates that `--limit` and `--last` are paired. `-t`/`--tag` can be given multiple times.
